...
// 💡 FIX: Ahora verificamos si algún producto tiene la etiqueta oculta "||" para saber si hay ADDON
const hasAddon = items.some((i) => i.image.includes("||"));
      ...
      <aside className="sticky top-6 space-y-0">
        <div className="bg-[#f0f0f0] rounded-t overflow-hidden">
          ...
          <div className="px-5 py-5 space-y-4 max-h-80 overflow-y-auto">
            {items.map((item) => {
              // 💡 FIX: Cortamos el texto por si vienen 2 fotos juntas
              ...
              return (
                <div key={item.id} className="flex items-start gap-3">
                  <div className="relative flex gap-1 flex-shrink-0">
                    ...
                    {/* Foto del Producto */}
                    <div className="relative w-14 h-16 bg-white border border-noir-gray-2 rounded overflow-hidden">
                      <img src={mainImg} alt={item.name} className="absolute inset-0 w-full h-full object-cover" />
                    </div>

                    {/* Foto del Logo Aliado (si existe) */}
                    {partnerLogo && (
                      <div className="relative w-14 h-16 bg-white border border-noir-gray-2 rounded flex items-center justify-center p-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.size && item.size !== "UNICO" && <p className="text-[10px] text-noir-gray-4 mt-0.5">{item.size}</p>}
                  </div>
