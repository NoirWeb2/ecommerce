...
import { Plus, Search, Edit, Trash2, Eye, X, Check, AlertTriangle, Tag, ChevronRight, Upload, ImageIcon, Loader2 } from "lucide-react";
...
export default function ProductosAdminPage() {
  ...
  const [createForm, setCreateForm] = useState<{ name: string; sku: string; price: number; stock: number; status: "ACTIVE" | "DRAFT" | "ARCHIVED"; categoryName: string; description: string; images: string[] }>({ name: "", sku: "", price: 0, stock: 0, status: "DRAFT", categoryName: "", description: "", images: [] });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setCreateOpen(false);
      setCreateForm({ name: "", sku: "", price: 0, stock: 0, status: "DRAFT", categoryName: "", description: "", images: [] });
                      ...
                      <tr key={p.id} className="hover:bg-noir-gray/30 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium line-clamp-2 max-w-[200px]">{p.name}</span>
                          </div>
                        </td>
