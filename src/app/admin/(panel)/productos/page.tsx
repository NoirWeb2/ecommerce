...
import { Plus, Search, Edit, Trash2, Eye, X, Check, AlertTriangle, Tag, ChevronRight, Upload, ImageIcon, Loader2, ToggleLeft, ToggleRight, Star, ShoppingCart } from "lucide-react";
...
function totalStock(variants: Variant[]) { return variants.reduce((sum, v) => sum + v.stock, 0); }

function ImageUploader({ images, onChange, single = false }: { images: string[]; onChange: (urls: string[]) => void; single?: boolean; }) {
const fileRef = useRef<HTMLInputElement>(null);
const [dragging, setDragging] = useState(false);
const [uploading, setUploading] = useState(false);

const uploadFile = useCallback(async (file: File): Promise<string | null> => {
  const formData = new FormData(); formData.append("file", file); formData.append("folder", "noir-lovers/products");
...
return (
  <div>
    <label className="block text-[10px] font-bold tracking-widest uppercase mb-2">
      {single ? "IMAGEN DE CATEGORÍA" : "IMÁGENES (Foto 1: Producto | Foto 2: Logo Aliado)"}
    </label>
    {images.length > 0 && (
      <div className="flex gap-2 flex-wrap mb-3">
        {images.map((url, i) => (
          <div key={i} className="relative w-20 h-24 bg-noir-gray overflow-hidden group">
            ...
            <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><X size={12} /></button>
          </div>
        ))}
      </div>
    )}
    {(!single || images.length === 0) && (
      <div
        onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); handleFiles(e.dataTransfer.files); }}
        onClick={() => !uploading && fileRef.current?.click()}
        className={`border-2 border-dashed transition-colors cursor-pointer flex flex-col items-center justify-center py-5 gap-2 ${dragging ? "border-noir-black bg-noir-gray" : "border-noir-gray-2 hover:border-noir-black"} ${uploading ? "opacity-60 cursor-not-allowed" : ""}`}
      >
        {uploading ? <><Loader2 size={20} className="text-noir-gray-4 animate-spin" /><p className="text-xs text-noir-gray-4">Subiendo...</p></> : <><Upload size={18} className="text-noir-gray-4" /><p className="text-xs text-noir-gray-4 text-center px-4">Arrastra una imagen aquí o haz clic</p></>}
      </div>
    )}
    <input ref={fileRef} type="file" accept="image/*" multiple={!single} className="hidden" onChange={(e) => e.target.files && handleFiles(e.target.files)} />
  </div>
);
}
...
const loadData = useCallback(async () => {
  setLoading(true);
  const [prodRes, catRes] = await Promise.all([ fetch("/api/admin/products", { cache: "no-store" }), fetch("/api/admin/categories", { cache: "no-store" }) ]);
...
const handleSaveEdit = async (e: React.FormEvent) => {
  e.preventDefault(); if (!editForm || !editProduct) return;
  setSaving(true);
  const res = await fetch(`/api/admin/products/${editProduct.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editForm) });
  if (res.ok) {
...
const handleDelete = async () => {
  ...
  if (res.ok) { setProducts((prev) => prev.filter((p) => p.id !== deleteId)); toast.success("Producto eliminado"); }
...
const handleCreateCategory = async () => {
  ...
  if (res.ok) { const d = await res.json(); setCategories((prev) => [...prev, d.category]); setNewCatName(""); toast.success(`Categoría "${name}" creada`); }
  else { const d = await res.json(); toast.error(d.error ?? "Error al crear categoría"); }
};

const handleSaveCatEdit = async (e: React.FormEvent) => {
  e.preventDefault(); if (!editCatForm) return;
  setSavingCat(true);
  const res = await fetch(`/api/admin/categories/${editCatForm.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: editCatForm.name, image: editCatForm.image }) });
  if (res.ok) { const d = await res.json(); setCategories((prev) => prev.map((c) => c.id === editCatForm.id ? d.category : c)); setEditCatForm(null); toast.success("Categoría actualizada"); }
...
const handleDeleteCat = async () => {
  ...
  if (res.ok) { setCategories((prev) => prev.filter((c) => c.id !== deleteCatId)); setDeleteCatId(null); toast.success("Categoría eliminada"); }
  else toast.error("Error al eliminar");
};
    ...
    <div className="flex gap-6 items-start">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" placeholder="Buscar por nombre o SKU..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full border border-noir-gray-2 pl-9 pr-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors bg-white" />
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <button onClick={() => setFilterCat("all")} className={`text-[10px] font-bold tracking-widest uppercase px-3 py-1.5 border transition-colors ${filterCat === "all" ? "bg-noir-black text-white border-noir-black" : "border-noir-gray-2 text-noir-gray-4 hover:border-noir-black hover:text-noir-black"}`}>Todos</button>
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setFilterCat(filterCat === cat.name ? "all" : cat.name)} className={`text-[10px] font-bold tracking-widest uppercase px-3 py-1.5 border transition-colors ${filterCat === cat.name ? "bg-noir-black text-white border-noir-black" : "border-noir-gray-2 text-noir-gray-4 hover:border-noir-black hover:text-noir-black"}`}>{cat.name}</button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-noir-gray-2 overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-16 gap-2 text-noir-gray-4 text-xs"><Loader2 size={16} className="animate-spin" /> Cargando productos...</div>
          ) : (
            <table className="w-full">
              ...
              <tbody className="divide-y divide-noir-gray">
                {filtered.map((p) => {
                  ...
                  const isAddon = p.tags?.includes("ADDON");
                  return (
                    <tr key={p.id} className="hover:bg-noir-gray/30 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="relative w-10 h-12 bg-noir-gray flex-shrink-0 overflow-hidden">
                            {p.images[0] ? <img src={p.images[0].url} alt={p.name} className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0 flex items-center justify-center"><ImageIcon size={14} className="text-noir-gray-4" /></div>}
                          </div>
                      ...
                      <td className="p-4">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${p.status === "ACTIVE" ? "bg-green-50 text-green-700 border border-green-200" : p.status === "DRAFT" ? "bg-gray-50 text-gray-600 border border-gray-200" : "bg-red-50 text-red-700 border border-red-200"}`}>
                          {p.status === "ACTIVE" ? "Activo" : p.status === "DRAFT" ? "Borrador" : "Archivado"}
                        </span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <button className="text-noir-gray-4 hover:text-noir-black transition-colors" title="Ver"><Eye size={14} /></button>
                          <button onClick={() => openEdit(p)} className="text-noir-gray-4 hover:text-noir-black transition-colors" title="Editar"><Edit size={14} /></button>
                          <button onClick={() => setDeleteId(p.id)} className="text-noir-gray-4 hover:text-red-600 transition-colors" title="Eliminar"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && !loading && (<tr><td colSpan={7} className="py-10 text-center text-xs text-noir-gray-4">Sin productos con este filtro</td></tr>)}
              </tbody>
            </table>
          )}
        </div>
      ...
      <div className="w-64 flex-shrink-0">
        <div className="bg-white border border-noir-gray-2">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-noir-gray-2"><Tag size={13} className="text-noir-gray-4" /><span className="text-[10px] font-black tracking-widest uppercase">CATEGORÍAS</span></div>
          <div className="divide-y divide-noir-gray">
            {categories.map((cat) => (
              <div key={cat.id} className="px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    {cat.image && <img src={cat.image} alt="" className="w-6 h-6 object-cover rounded-sm border border-noir-gray-2" />}
                    <div>
                      <button onClick={() => setFilterCat(filterCat === cat.name ? "all" : cat.name)} className="text-xs font-bold text-left truncate w-full hover:text-noir-gray-4 transition-colors flex items-center gap-1">
                        {filterCat === cat.name && <ChevronRight size={10} className="flex-shrink-0" />} {cat.name}
                      </button>
                      <p className="text-[9px] text-noir-gray-4 mt-0.5">{cat._count.products} producto{cat._count.products !== 1 ? "s" : ""} · /{cat.slug}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => setEditCatForm({ id: cat.id, name: cat.name, image: cat.image || "" })} className="text-noir-gray-4 hover:text-noir-black transition-colors" title="Editar"><Edit size={11} /></button>
                    <button onClick={() => setDeleteCatId(cat.id)} className="text-noir-gray-4 hover:text-red-600 transition-colors" title="Eliminar"><Trash2 size={11} /></button>
                  </div>
                </div>
              </div>
            ))}
            {categories.length === 0 && !loading && <p className="px-4 py-3 text-[10px] text-noir-gray-4">Sin categorías aún.</p>}
          </div>
          <div className="border-t border-noir-gray-2 p-4">
            ...
            <input type="text" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleCreateCategory()} placeholder="Nombre de la categoría" className="w-full border border-noir-gray-2 px-3 py-2 text-xs outline-none focus:border-noir-black transition-colors mb-2" />
            <button onClick={handleCreateCategory} className="w-full flex items-center justify-center gap-1.5 bg-noir-black text-white py-2 text-[10px] font-black tracking-widest uppercase hover:bg-black transition-colors"><Plus size={11} /> CREAR</button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="relative bg-white w-full max-w-sm p-6">
          ...
          <form onSubmit={handleSaveCatEdit} className="space-y-4">
            <div>
              ...
              <input required type="text" value={editCatForm.name} onChange={(e) => setEditCatForm({ ...editCatForm, name: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors" />
            </div>
            <ImageUploader single={true} images={editCatForm.image ? [editCatForm.image] : []} onChange={(urls) => setEditCatForm({ ...editCatForm, image: urls[0] || "" })} />
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={savingCat} className="flex-1 flex justify-center items-center gap-2 bg-noir-black text-white px-4 py-3 text-xs font-bold tracking-widest uppercase hover:bg-black transition-colors disabled:opacity-60">
                {savingCat ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} GUARDAR
              </button>
            </div>
          </form>
        </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
          ...
          <form onSubmit={handleSaveEdit} className="space-y-4">
            <div><label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">NOMBRE</label><input required type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors" /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">SKU</label><input type="text" value={editForm.sku} onChange={(e) => setEditForm({ ...editForm, sku: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black font-mono transition-colors" /></div>
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">CATEGORÍA</label>
                <select value={editForm.categoryName} onChange={(e) => setEditForm({ ...editForm, categoryName: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors bg-white">
                  <option value="">— Sin categoría —</option>{categories.map((c) => (<option key={c.id} value={c.name}>{c.name}</option>))}
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={saving} className="flex items-center gap-2 bg-noir-black text-white px-7 py-3 text-xs font-bold tracking-widest uppercase hover:bg-black transition-colors disabled:opacity-60">
                {saving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} {saving ? "GUARDANDO..." : "GUARDAR CAMBIOS"}
              </button>
              <button type="button" onClick={() => setEditProduct(null)} className="border border-noir-gray-2 px-6 py-3 text-xs font-bold tracking-widest uppercase hover:border-noir-black transition-colors">CANCELAR</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-xl max-h-[90vh] overflow-y-auto p-6">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div><label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">NOMBRE *</label><input required type="text" value={createForm.name} onChange={(e) => setCreateForm({ ...createForm, name: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors" /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">SKU</label><input type="text" value={createForm.sku} onChange={(e) => setCreateForm({ ...createForm, sku: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black font-mono transition-colors" /></div>
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase mb-1.5">CATEGORÍA</label>
                <select value={createForm.categoryName} onChange={(e) => setCreateForm({ ...createForm, categoryName: e.target.value })} className="w-full border border-noir-gray-2 px-4 py-2.5 text-sm outline-none focus:border-noir-black transition-colors bg-white">
                  <option value="">— Sin categoría —</option>{categories.map((c) => (<option key={c.id} value={c.name}>{c.name}</option>))}
                </select>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={creating} className="flex items-center gap-2 bg-noir-black text-white px-7 py-3 text-xs font-bold tracking-widest uppercase hover:bg-black transition-colors disabled:opacity-60">
                {creating ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} {creating ? "CREANDO..." : "CREAR PRODUCTO"}
              </button>
              <button type="button" onClick={() => setCreateOpen(false)} className="border border-noir-gray-2 px-6 py-3 text-xs font-bold tracking-widest uppercase hover:border-noir-black transition-colors">CANCELAR</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-sm p-6 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={handleDelete} disabled={deleting} className="bg-red-600 text-white px-6 py-2.5 text-xs font-bold tracking-widest uppercase hover:bg-red-700 transition-colors disabled:opacity-60">SÍ, ELIMINAR</button>
