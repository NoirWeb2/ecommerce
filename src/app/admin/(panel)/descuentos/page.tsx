...
import { Plus, Tag, Copy, Trash2, Edit, X, Check, AlertTriangle } from "lucide-react";
...
const INITIAL_COUPONS: Coupon[] = [
  { id: "1", code: "BIENVENIDO10", type: "PERCENTAGE", value: 10, usedCount: 5, maxUses: 100, isActive: true, endDate: "2025-12-31" },
  { id: "2", code: "ENVIOGRATIS", type: "FREE_SHIPPING", value: 0, usedCount: 23, maxUses: null, isActive: true, endDate: null },
  { id: "3", code: "NL50K", type: "FIXED", value: 50000, usedCount: 12, maxUses: 50, isActive: false, endDate: "2025-05-31" },
];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editId) {
      setCoupons((prev) =>
        prev.map((c) =>
          c.id === editId
            ? { ...c, code: form.code, type: form.type, value: Number(form.value), maxUses: form.maxUses ? Number(form.maxUses) : null, endDate: form.endDate || null }
            : c
        )
      );
            ...
            {coupons.map((c) => (
              <tr key={c.id} className="hover:bg-noir-gray/30 transition-colors">
                ...
                <td className="p-4 text-sm text-noir-gray-4">
                  {c.type === "PERCENTAGE" ? `${c.value}% descuento` :
                   c.type === "FIXED" ? `$${c.value.toLocaleString("es-CO")} descuento` :
                   "Envío gratis"}
                </td>
                ...
                <td className="p-4">
                  <button
                    ...
                    className={`text-[10px] font-bold px-2 py-0.5 rounded border transition-colors ${c.isActive ? "bg-green-50 text-green-700 border-green-200 hover:bg-green-100" : "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100"}`}
                  >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input required type="text" placeholder="DESCUENTO20"
                    value={form.code}
                    onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })}
                    className="flex-1 border border-noir-gray-2 px-3 py-2.5 text-sm outline-none focus:border-noir-black font-mono uppercase transition-colors" />
                  <button type="button"
                    onClick={() => setForm({ ...form, code: Math.random().toString(36).substring(2, 10).toUpperCase() })}
                    className="border border-noir-gray-2 px-3 py-2.5 text-xs hover:border-noir-black transition-colors">
                    AUTO
                  </button>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as typeof form.type })}
                    className="w-full border border-noir-gray-2 px-3 py-2.5 text-sm outline-none focus:border-noir-black bg-white transition-colors">
                ...
                {form.type !== "FREE_SHIPPING" && (
                  <div>
                    ...
                    <input type="number" placeholder={form.type === "PERCENTAGE" ? "20" : "50000"}
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="flex items-center gap-2 bg-noir-black text-white px-7 py-3 text-xs font-bold tracking-widest uppercase hover:bg-black transition-colors">
                  <Check size={13} /> {editId ? "GUARDAR CAMBIOS" : "CREAR CUPÓN"}
                </button>
                <button type="button" onClick={() => setShowForm(false)} className="border border-noir-gray-2 px-6 py-3 text-xs font-bold tracking-widest uppercase hover:border-noir-black transition-colors">
                  CANCELAR
                </button>
              </div>
            </form>
